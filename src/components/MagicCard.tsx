...
const createParticleElement = (x: number, y: number, color: string) => {
    ...
    box-shadow: 0 0 8px rgba(${color}, 0.8), 0 0 4px white;
    pointer-events: none;
    z-index: 100;
    left: ${x}px;
    top: ${y}px;
...
export const BentoCardGrid = ({ children, className = '' }: { children: ReactNode; className?: string }) => {
    const gridRef = useRef(null);

    return (
        <>
            <GlobalSpotlight gridRef={gridRef} glowColor="178, 197, 157" spotlightRadius={450} />
...
export const MagicCard = ({
    ...
    useEffect(() => {
        if (disableAnimations || !cardRef.current || window.innerWidth <= MOBILE_BREAKPOINT) return;
        ...
        border-radius: 50%;
        background: radial-gradient(circle, rgba(${glowColor}, 0.5) 0%, rgba(${glowColor}, 0.2) 20%, transparent 60%);
